...
Playwright MCP is configured for this project (see `.claude/settings.json`). Use it to self-verify UI changes against the local dev server before considering any frontend task done.

### Stack context
- Frontend: static site (`index.html`) deployed to Cloudflare Pages, watched from GitHub repo
- Local dev server: `npm run dev` (Vite), runs at `localhost:5173`
- Auth: Supabase (login required for favorites/watchlist features)
- Stats JSON files are generated by GitHub Actions and committed to the repo

### How to use Playwright MCP for self-QA
1. Start the dev server: `npm run dev`
2. After making UI changes, use the Playwright MCP browser to navigate to `localhost:5173` and verify the changes look and behave correctly
3. Always say "use playwright mcp" explicitly on the first browser interaction in a session, otherwise bash may be used instead

### What to verify after UI changes
- Player search renders results correctly
- Player cards/rows display stats fields without layout breaks
- Favorites/watchlist UI works (may require login — navigate to login page first and authenticate manually, then continue)
- Responsive layout at 375px (mobile) and 1280px (desktop)
- No broken images or console errors visible in accessibility snapshot

### Verification pass steps
1. Open `localhost:5173`
2. Walk through the affected feature in plain language
3. Take a screenshot at mobile (375px) and desktop (1280px) viewports
4. Report what passed, what looked off, and any console errors

**Do not wait to be asked** — after any significant UI change, run a Playwright MCP verification pass automatically and report results before considering the task done.
